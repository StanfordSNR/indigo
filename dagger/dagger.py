...
class DaggerLeader(object):
    def __init__(self, cluster, server, worker_tasks):
        ...
        self.sync_op = tf.group(*[v1.assign(v2)
                                  for v1, v2 in zip(cpu_vars, gpu_vars)])
        ...
        for idx in worker_tasks:
            ...
            self.sync_queues[idx] = tf.FIFOQueue(
                3, [tf.int16], shared_name=queue_name)
    ...
    def setup_tf_ops(self, server):
        ...
        git_commit = check_output(
            'cd %s && git rev-parse @' % context.base_dir, shell=True)
    ...
    def wait_on_workers(self):
        """ Update which workers are done or dead. Stale tokens will eventually
        be cleaned out.
    ...
    def get_soft_targets(self, ep_cwnds):
        ...
        for i in range(len(ep_cwnds)):  # sender max steps
            ...
            action_distance = np.array(
                [max(float(Policy.min_cwnd), curr_cwnd / 2.0),
                 max(float(Policy.min_cwnd), curr_cwnd - 10.0),
                 curr_cwnd,
                 min(float(Policy.max_cwnd), curr_cwnd + 10.0),
                 min(float(Policy.max_cwnd), curr_cwnd * 2.0)])
            action_distance = -np.abs(action_distance - best_cwnd)
    ...
    def run_one_train_step(self, batch_states, batch_soft_targets,
                           batch_actions):
        ...
        ret = self.sess.run(ops_to_run,
                            feed_dict={pi.input: batch_states,
    ...
    def train(self):
        ...
        while curr_epoch <= MAX_EPOCH:
            ...
            for batch_num in np.random.permutation(num_batches):
                ...
                loss = self.run_one_train_step(
                    batch_states, batch_soft_targets, batch_actions)
        ...
        for curr_ep in xrange(self.max_eps):
            ...
            if workers_ep_done > 0:
                while True:
                    ...
                    # data[0] = aug_state: [...]
                    # data[1] = action: [current cwnd, best cwnd, best action]
                    ...
                    self.aggregated_soft_targets.append(
                            self.get_soft_targets(this_ep_cwnds))
                    self.aggregated_actions.append(
                            [int(foo[2]) for foo in data[1]])
...
class DaggerWorker(object):
    ...
    def run(self, debug=False):
        ...
        while True:
            ...
            ret = self.rollout()
            if ret == -1:
                sys.stderr.write('env rollout error, restart env...\n')
                continue

            if debug:
                queue_size = self.sess.run(self.train_q.size())
                sys.stderr.write(
                    '[WORKER %d EP %d]: enqueueing a sequence of data into '
                    'queue of size %d\n' %
